 ...
 */
class UriTool(private val context: Context) {
    ...
    fun revokePersistableUriPermission(uri: Uri) {
        try {
     ...
     * @return 存在すれば true
     */
    suspend fun existsContentUri(uri: Uri): Boolean = withContext(Dispatchers.IO) {
        // Uri が存在するかを返す処理はない
